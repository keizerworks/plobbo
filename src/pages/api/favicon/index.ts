...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!organizationId || !userId) {
      return new Response(
        JSON.stringify({ error: "Organization ID and User ID are required" }),
        {
          status: 400,
        },
      );
    ...
    if (!file) {
      return new Response(
        JSON.stringify({ error: "No favicon file provided" }),
        {
          status: 400,
        },
      );
    ...
    if (!user) {
      return new Response(
        JSON.stringify({
          error: "Unauthorized. Only organization admins can upload favicons.",
        }),
        {
          status: 403,
        },
